...
const getPersonalizedAdvice = (product) => {
  ...
  if (score >= 80) {
    advice.push({
      ...
      text: `Ce produit est de tres bonne qualite pour ${animalName}. Vous pouvez le donner en toute confiance.`,
      type: 'success',
    });
  } else if (score >= 60) {
    advice.push({
      icon: 'thumbs-up',
      title: 'Bon produit',
      text: `Ce produit est correct pour ${animalName}, mais il existe des alternatives encore meilleures.`,
      type: 'info',
    });
  } else if (score >= 40) {
    advice.push({
      ...
      text: `Ce produit contient des ingredients discutables. Privilegiez des produits avec un score superieur a 60.`,
      type: 'warning',
    });
  } else {
    advice.push({
      icon: 'alert-triangle',
      title: 'Produit deconseille',
      text: `Ce produit est de mauvaise qualite pour ${animalName}. Nous recommandons de chercher une meilleure alternative.`,
      type: 'error',
    });
  }

  // Conseils sur les ingrédients dangereux
  if (dangerousIngs.length > 0) {
    const names = dangerousIngs.slice(0, 3).map(i => i.name).join(', ');
    advice.push({
      icon: 'x-circle',
      title: 'Ingredients a risque detectes',
      text: `Ce produit contient : ${names}. Ces ingredients peuvent etre nocifs a long terme. Verifiez aupres de votre veterinaire.`,
      type: 'error',
    });
  }

  // Conseils sur les additifs
  if (dangerousAdds.length > 0) {
    advice.push({
      icon: 'slash',
      title: `${dangerousAdds.length} additif${dangerousAdds.length > 1 ? 's' : ''} dangereux`,
      text: `Les additifs comme ${dangerousAdds[0].name || dangerousAdds[0].code} sont controverses et potentiellement cancerigenes. Evitez les produits qui en contiennent.`,
      type: 'error',
    });
  }
  ...
  if (details.protein != null) {
    if (details.protein >= 5) {
      advice.push({
        ...
        text: targetAnimal === 'chat'
          ? 'Les chats sont des carnivores stricts et ont besoin d\'un apport eleve en proteines. Ce produit repond bien a ce besoin.'
          : 'Bon apport en proteines animales, essentiel pour la masse musculaire et l\'energie.',
        type: 'success',
      });
    } else if (details.protein <= 0 && score < 70) {
      advice.push({
        icon: 'trending-down',
        title: 'Faible en proteines',
        text: `${animalName.charAt(0).toUpperCase() + animalName.slice(1)} a besoin de proteines animales de qualite. Cherchez un produit avec de la vraie viande en premier ingredient.`,
        type: 'warning',
      });
    }
  }
  ...
  if (details.fat != null && details.fat < 0) {
    advice.push({
      icon: 'disc',
      title: 'Taux de gras eleve',
      text: `Un exces de matieres grasses peut entrainer de l'obesite et des problemes pancreatiques. A donner avec moderation.`,
      type: 'warning',
    });
  }
  ...
  if (details.fiber != null && details.fiber >= 3) {
    advice.push({
      icon: 'layers',
      title: 'Bon apport en fibres',
      text: 'Les fibres favorisent une bonne digestion et un transit regulier. Excellent point pour ce produit.',
      type: 'success',
    });
  }

  // Conseil premier ingrédient
  if (ingredients.length > 0) {
    ...
    if (isMeat) {
      advice.push({
        icon: 'check-circle',
        title: 'Viande en 1er ingredient',
        text: 'Le premier ingredient est une source de proteine animale, signe d\'un produit de qualite superieure.',
        type: 'success',
      });
    } else if (score < 70) {
      ...
      if (isCereal) {
        advice.push({
          icon: 'info',
          title: 'Cereales en 1er ingredient',
          text: 'Le premier ingredient est une cereale, pas une proteine animale. Les animaux carnivores digerent mal les cereales en grande quantite.',
          type: 'warning',
        });
      }
    }
  ...
  // Conseil animal spécifique
  if (targetAnimal === 'chat' && score >= 50) {
    advice.push({
      icon: 'droplet',
      title: 'Hydratation du chat',
      text: 'Les chats boivent naturellement peu. Alternez croquettes et patee pour assurer une bonne hydratation.',
      type: 'info',
    });
  }
  ...
  if (targetAnimal === 'chien' && score >= 50) {
    advice.push({
      icon: 'clock',
      title: 'Conseil de distribution',
      text: 'Divisez la ration quotidienne en 2 repas. Evitez l\'exercice intense 1h apres le repas pour prevenir la torsion d\'estomac.',
      type: 'info',
    });
  }
