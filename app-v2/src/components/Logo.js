...
export const PawIcon = ({ size = 32, color, gradient = false }) => {
  const fillColor = gradient ? 'url(#pawGrad)' : (color || '#FFF');
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 64 64" fill="none">
      ...
      <Path
        ...
        fill={fillColor}
      />
      <Circle cx="17" cy="17" r="6.5" fill={fillColor} />
      <Circle cx="47" cy="17" r="6.5" fill={fillColor} />
      <Circle cx="10.5" cy="31" r="5.8" fill={fillColor} />
      <Circle cx="53.5" cy="31" r="5.8" fill={fillColor} />
    </Svg>
  );
};
...
const Logo = ({ variant = 'light', size = 'md', showText = true, style }) => {
  ...
  const clr = COLOR_CONFIG[variant] ?? COLOR_CONFIG.light;
